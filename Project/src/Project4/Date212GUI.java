...
public class Date212GUI extends JFrame { 
	...
	public static Date212GUI initiateGUI(Date212GUI myGUI) {
		...
		myGUI.setLocation(350, 200); //display the screen in the center
	...
	public static void printFileToGUI(String[] list, Date212GUI dg, TextArea sorted, TextArea unsorted, TextArea invalid){		
		...
		//TreeMap<Integer, Date212> myUnsortedDateList = new TreeMap<Integer, Date212>();
		
		for (int i = 0; i < list.length; i++) {
			if(isValid(list[i]))
			mySortedDateMap.put(Integer.parseInt(list[i]), new Date212(list[i]));

//			else if(isValid(list[i]) == false) {
//				throw new IllegalDate212Exception(list[i] + " is invalid date.\n");
//			}
		}
		...
		while(i.hasNext()) {
			...
			sorted.append(me.getValue().getNameOfDay() + ", " + (me.getValue()).toString(me.getValue()) + "\n");
		}
